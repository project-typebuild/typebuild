...
def create_llm_output(selected_res_project):
    ...
    # Get the full text and the res for it.  Add it to the first row.
            ...
            # Show len of full text
            # Every chunk is added as a row to the output column.
            chunks_analyzed = len(df[~df[output_col_name].isna()])
            # Num chunks if we chunk by 6000 characters
            chunks = chunk_text(full_text, max_chars=6000)
            progress_indicator = st.empty()
            if chunks_analyzed == 0:
                progress_text = "Analysis yet to start..."
            else:
                progress_text = f"{chunks_analyzed} /{len(chunks)} chunks analyzed so far"
            progress_indicator.progress(int(chunks_analyzed/len(chunks)), progress_text)
        else:
                ...
                with st.spinner("Analyzing the rest of the data..."):
                    if consolidated:
                        ...
                        for i, chunk in enumerate(chunks):
                            ...
                            progress_indicator.progress(i/len(chunks), progress_text)
                            if i >= chunks_analyzed:
                                res_text = None
                                try:
                                    res_text = get_llm_output(
                                        messages=[
                                ...
                                # If there is a runtime error, try again
                                except Exception as e:
                                    
                                    if 'time' in str(e).lower():
                                        with st.spinner("Got a runtime error.  Trying again..."):
                                            # Wait for 30 seconds
                                            time.sleep(30)
                                            res_text = get_llm_output(
                                                messages=[
                                ...
                                if res_text:
                                    df.iloc[i, df.columns.get_loc(output_col_name)] = res_text
                                    df.to_parquet(selected_table, index=False)
                                else:
                                    break
...
def row_by_row_llm_res(text_or_list, system_instruction, sample=False, word_limit=1000, model='claude-2'):
    ...
    if not text:
        ...
        for i, chunk in enumerate(chunks):
            progress_text = f"Analyzing chunk {i+1} of {len(chunks)}"
            progress_bar.progress(int(i/len(chunks)), progress_text)
            max_tokens = int(word_limit * 4/3)
    ...
    progress_bar.progress(1.0, "Done.  Consolidating...")
    time.sleep(3)
    return "\n".join(output)
